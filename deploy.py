...
def upload_fileorg(bucket_name,filePath):
    session = boto3.Session(profile_name='artlisa')
...
[zipFile(x,stagingDir + x) if isZipFile(x) else copyFile(x,stagingDir + x) for x in getFiles('dist')]
[upload_fileorg(bucketName,x) for x in getFiles(stagingDir)]